...
func (c *AuthController) AuthorizeUser(ctx *gin.Context) {
	...
	token, err := c.as.Authorize(ctx, credentials.Username, credentials.Password)
	if err != nil {
...
func (c *AuthController) ChangePassword(ctx *gin.Context) {
	...
	if err := c.as.ChangePassword(ctx, username, passwords.OldPassword, passwords.NewPassword); err != nil {
		ctx.JSON(int(err.StatusCode), err)
		return
	}
